...
class TestSchedule < Test::Unit::TestCase
  ...
  test "#convert_ice_cube_occurrence" do
    ...
    bunk_timezoned_event.start.ical_param("tzid", "GMT-08.00/-07.00") # force bad timezone
  ...
  test "#parse_ical_byday" do
    assert_equal({day_code: "SA", position: 1}, Schedule.new(nil).parse_ical_byday("1SA"))
    assert_equal({day_code: "MO", position: 0}, Schedule.new(nil).parse_ical_byday("MO"))
  end
